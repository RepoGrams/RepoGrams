...
\begin{description}[leftmargin=!,labelwidth=\widthof{\bfseries Frequency of use:}]
	\item[Use-case:] Trying to visualize a Git repository which is protected by a private SSH key
	...
	\item[Scenario:]
		\begin{enumerate}[leftmargin=1.5em]
			...
			\item The user enters an URL that points to a Git repository which is protected by a private SSH key.
			\item The user clicks on \enquote{Visualize}.
			\item The website shows an error message below the input field:
				\enquote{The Git repository has to be without SSH key protection.}.
		\end{enumerate}
