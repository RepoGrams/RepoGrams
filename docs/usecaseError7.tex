...
\begin{description}[leftmargin=!,labelwidth=\widthof{\bfseries Frequency of use:}]
	\item[Use-case:] Trying to visualize a Git repository which is too big
	...
	\item[Scenario:]
		\begin{enumerate}[leftmargin=1.5em]
			...
			\item The user enters an URL that points to an Git repository which is too big.
			...
			\item The website shows an error message below the input field:
				\enquote{The repository is too big. Please try another one.}.
		\end{enumerate}

	\item[Priority:] high
	\item[Frequency of use:] Seldom -- only occurs if the repository is too big for the system to handle
