...
RUN echo "daemon off;" >> /etc/nginx/nginx.conf && nginx -t
RUN mkdir -p /var/log/supervisor
COPY conf/supervisord.conf /etc/supervisor/conf.d/supervisord.conf

# Fill in placeholders in the served files
RUN sed -i "s/@@@BUILDINFO@@@/`cat /var/www/html/.buildinfo`/" /var/www/html/index.html
RUN sed -i "s/@@@BUILDDATE@@@/`cat /var/www/html/.builddate`/" /var/www/html/fse2015/index.html
