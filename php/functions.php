...
function render($commit, $id, $width){
	...
	if($floorDiff < $ceilDiff){ //we want to floor the value
		...
		$head = '<li class="customBlock repoblock" id="'.$id.'" '.$style.' '.$tooltip.'>';
		echo ($head);
		$width += $floorValue;
		...
		$head = '<li name="repoblock" class="customBlock repoblock" id="'.$id.'" '.$style.' '.$tooltip.'>';
		echo ($head);
		$width += $ceilValue;
	}
	$end = '</li>';
...
function renderLast($commit, $count, $width){
	...
	$tooltip = 'data-html="true" data-original-title="Author: '.$commit[5].'<br>
			                                          Date: '. $datum.'<br>
			                                          Comment: '.$commit[3].'" data-placement="right" rel="tooltip"';
	$head = '<li name="customBlock repoblock" class="repoblock" id="'.$count.'" '.$style.' '.$tooltip.'>';
	$end = '</li>';
	echo ($head);
	echo ($end);
}
...
function renderLegend(){
	...
	for ( $i = 0; $i < count($legend); $i++)
	{
		$key = $legend[$i][0];
		$val = $legend[$i][1];
		$colorBlock = '<div style="line-height:1"><div class="customBlock repoblock" onmouseleave="disablehighlight(this);" onmouseover="highlight(this);" style="background-color:rgb('.ceil($val[0]).','.ceil($val[1]).','.ceil($val[2]).');width:15px;height:14px;">';
